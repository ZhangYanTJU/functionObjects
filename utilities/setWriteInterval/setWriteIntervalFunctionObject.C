...
{
    Info<<"timeOutputValue = "<<time_.timeOutputValue()<<endl; // CA
    Info<<"writeInterval(CA) = "<<writeIntervalPtr_().value(time_.timeOutputValue())<<endl; //CA
    const_cast<Time&>(time_).setWriteInterval
    (
        //writeIntervalPtr_().value(time_.timeOutputValue())
        time_.userTimeToTime(writeIntervalPtr_().value(time_.timeOutputValue()))
    );
